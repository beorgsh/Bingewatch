...
export default function HeroBanner({
  ...
  // Combine media or mediaList into an active pool
  const activeItems = mediaList.length > 0 ? mediaList : media ? [media] : [];

  // Cycle every 2.5 seconds (2500ms) with fade transition
  useEffect(() => {
  ...
  return (
    <div className="relative h-[70vh] sm:h-[82vh] lg:h-[88vh] w-full select-none overflow-hidden bg-black">
      {/* Background Media Backdrop with Fade Animation on Key Change */}
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div
            ...
            className="absolute inset-0 w-full md:w-3/4 pointer-events-none"
          ></div>
          <div
            style={{
              background:
      ...
      {/* Hero Content Overlay with Fade Animation */}
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col justify-end pb-10 sm:pb-20 pointer-events-none"
        >
      ...
      {/* Bottom Right Mute & Maturity Rating Box */}
      <div className="absolute right-0 bottom-12 sm:bottom-20 z-20 flex items-center gap-2 sm:gap-3 pr-3 sm:pr-8">
        <button
        ...
        <div className="bg-neutral-900/90 border-l-2 border-white px-2 sm:px-3 py-1 text-[10px] sm:text-xs text-neutral-300 font-semibold tracking-wider">
